...
class ThreadTestCase(TestCase):
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, contenido="Holines")
        message2 = Message.objects.create(user=self.user2, contenido="Chaval")
        self.thread.messages.add(message1, message2)
        self.assertEqual(len(self.thread.messages.all()), 2)
        
        for message in self.thread.messages.all():
            print(f"({message.user}): {message.contenido}")
            
    def test_add_message_from_user_not_in_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, contenido="Holines")
        message2 = Message.objects.create(user=self.user2, contenido="Chaval")
        message3 = Message.objects.create(user=self.user3, contenido="Hola tios jaja")
